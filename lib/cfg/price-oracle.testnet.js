"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = {
    fetchPricesInterval: Number(process.env.FETCH_PRICE_INTERVAL_IN_MS) || 60000,
    minPriceChangePercentage: Number(process.env.UPDATE_PRICE_CHANGE_PCT) || 2,
    maxPriceChangePercentage: Number(process.env.UPDATE_PRICE_CHANGE_PCT_CAP) || 25,
    pricePrecision: 8,
    supportedTokens: [
        {
            chainId: 2,
            tokenId: "dai",
            tokenContract: "00000000000000000000000011fE4B6AE13d2a6055C8D9cF65c55bac32B5d844",
        },
        {
            chainId: 2,
            tokenId: "ethereum",
            tokenContract: "000000000000000000000000B4FBF271143F4FBf7B91A5ded31805e42b2208d6",
        },
        {
            chainId: 4,
            tokenId: "binancecoin",
            tokenContract: "000000000000000000000000ae13d989daC2f0dEbFf460aC112a837C89BAa7cd",
        },
        {
            chainId: 5,
            tokenId: "matic-network",
            tokenContract: "0000000000000000000000009c3C9283D3e44854697Cd22D3Faa240Cfb032889",
        },
        {
            chainId: 6,
            tokenId: "avalanche-2",
            tokenContract: "000000000000000000000000d00ae08403B9bbb9124bB305C09058E32C39A48c",
        },
        {
            chainId: 6,
            tokenId: "usd-coin",
            tokenContract: "0000000000000000000000005425890298aed601595a70AB815c96711a31Bc65",
        },
        {
            chainId: 10,
            tokenId: "fantom",
            tokenContract: "000000000000000000000000f1277d1Ed8AD466beddF92ef448A132661956621",
        },
        {
            chainId: 14,
            tokenId: "celo",
            tokenContract: "000000000000000000000000F194afDf50B03e69Bd7D057c1Aa9e10c9954E4C9",
        },
    ],
};
//# sourceMappingURL=price-oracle.testnet.js.map